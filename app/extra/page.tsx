...
const strava_single_activity_endpoint =
...
export default function ExtrasPage() {
  ...
  useEffect(() => {
    ...
    const getProfile = async () => {
      const athleteURL =
        strava_athlete_stats_endpoint + MY_ATHLETE_ID + "/stats";
              ...
              <p className="pl-3 mt-4 leading-8 duration-150 text-zinc-400 group-hover:text-zinc-300">
                Distance:{" "}
                {
                  myStravaProfile.distance ? (
                    `${(myStravaProfile.distance * 0.000621371).toFixed(
                      2
                    )} miles`
                  ) : (
                    <Box sx={{ display: "flex" }}>
              ...
              ultraFiddy.name ? (
                <>
                  ...
                  <p className="text-center mt-4 leading-8 duration-150 text-zinc-400 group-hover:text-zinc-300">
                    Distance: {(ultraFiddy.distance * 0.000621371).toFixed(2)}{" "}
                    miles
                  </p>
                  <p className="text-center mt-4 leading-8 duration-150 text-zinc-400 group-hover:text-zinc-300">
                    Elevation Gain:{" "}
                    {Math.round(ultraFiddy.total_elevation_gain * 3.28084)} ft
                  </p>
          ...
          <Card>
            {
              ultraSixtyK.name ? (
                <>
                  ...
                  <p className="text-center mt-4 leading-8 duration-150 text-zinc-400 group-hover:text-zinc-300">
                    Distance: {(ultraSixtyK.distance * 0.000621371).toFixed(2)}{" "}
                    miles
                  </p>
                  <p className="text-center mt-4 leading-8 duration-150 text-zinc-400 group-hover:text-zinc-300">
                    Elevation Gain:{" "}
                    {Math.round(ultraSixtyK.total_elevation_gain * 3.28084)} ft
                  </p>
                  <p className="text-center my-4 leading-8 duration-150 text-zinc-400 group-hover:text-zinc-300">
